...
class Attention(nn.Module):
    def __init__(self, attention_rnn_dim, embedding_dim, attention_dim,
        ...
        if hparams.model_type == "multi-speaker":
...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        if self.num_speakers > 1:
                speaker_embed_table = tf.get_variable('speaker_embedding',[self.num_speakers, hparams.speaker_embedding_size], dtype=tf.float32,initializer=tf.truncated_normal_initializer(stddev=0.5))
                # [N, T_in, speaker_embedding_size]
                speaker_embed = tf.nn.embedding_lookup(speaker_embed_table, hparams.speaker_id)                       
                
                deep_dense = lambda x, dim,name: tf.layers.dense(x, dim, activation=tf.nn.softsign,name=name)   # softsign: x / (abs(x) + 1)

                encoder_rnn_init_state = deep_dense( speaker_embed, hparams.encoder_lstm_units * 4,'encoder_init_dense')  # hparams.encoder_lstm_units = 256

                decoder_rnn_init_states = [deep_dense(speaker_embed, hparams.decoder_lstm_units*2,'decoder_init_dense_{}'.format(i)) for i in range(hparams.decoder_layers)]  # hparams.decoder_lstm_units = 1024
...
class Tacotron2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        ###############################
        transcript_outputs = transcript_outputs.cuda()
        ################################
